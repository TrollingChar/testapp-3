...
class LandGen {
    ...
    public void Expand (int iterations = 1) {
        for (int i = 0; i < iterations; ++i) {
            int w = array.GetLength(0),
                h = array.GetLength(1),
                resultw = w * 2 - 1,
                resulth = h * 2 - 1;
            byte[,] result = new byte[w * 2 - 1, h * 2 - 1];
            ...
            for (int x = 1; x < resultw; x += 2) {
                for (int y = 1; y < resulth; y += 2) {
                    result[x, y] = (byte)(RNG.Bool(result[x - 1, y - 1] +
                                                   result[x + 1, y - 1] +
                                                   result[x + 1, y + 1] +
                                                   result[x - 1, y + 1], 4) ? 1 : 0);
                }
            }
            ...
            for (int x = 1; x < resultw - 1; ++x) {
                for (int y = ((x & 1) != 0) ? 2 : 1; y < resulth - 1; y += 2) {
                    result[x, y] = (byte)(RNG.Bool(result[x - 1, y] +
                                                   result[x + 1, y] +
                                                   result[x, y - 1] +
                                                   result[x, y + 1], 4) ? 1 : 0);
                }
    ...
    public void Cellular(uint rules, int iterations = 5) {
        uint[] init = {1, 1 << 16};
        int w = array.GetLength(0);
